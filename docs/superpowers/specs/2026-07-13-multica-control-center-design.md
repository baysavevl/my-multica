...
This repository is a dedicated personal Multica control-center project. It contains a React/Vite frontend that can deploy to Vercel as a static app and a local Spring Boot bridge that talks to the authenticated `multica` CLI on the developer machine.

The requested tool is a local control panel for managing and orchestrating real Multica agents. It should not replace Multica. It should make the important operating concepts visible in this personal repo and provide a narrow, safe bridge for common Multica CLI workflows.
...
The first version is successful when a developer can open the app, understand the active Multica setup, create a project, create an issue, assign it to a real local agent, and see enough status and command output to debug the path without leaving the personal control center.
